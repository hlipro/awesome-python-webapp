...
class _ConnectionCtx(object):
    ...
    with connection():
        ...
        with connection():
            pass
    '''
    #定义了__enter__和__exit__方法的类可以用with语句调用，可以保证__exit__一定会被调用
    def __enter__(self):
        global _db_ctx
...
class _TransactionCtx(object):
    ...
    def __exit__(self, exctype, excvalue, traceback): #exctype: exception type
        global _db_ctx
        _db_ctx.transactions = _db_ctx.transactions - 1
        try:
            if _db_ctx.transactions==0:
                if exctype is None: #if no error
                    self.commit()
                else:
                    self.rollback()
        finally:
...
def with_transaction(func):
    ...
    def _wrapper(*args, **kw):
        ...
        with _TransactionCtx():
            return func(*args, **kw)
        _profiling(_start) # 我们可以在return之后继续执行语句
    return _wrapper
...
def _select(sql, first, *args):
    ...
    try:
        ...
        if cursor.description:
            ...
            #cursor.description contains a list of discriptions for found rows
            #each discription contains name, type_code, display_size, internal_size, precision, scale, null_ok
            #so x[0] corresponds to the name of each discription
        if first:
